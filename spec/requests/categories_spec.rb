...
RSpec.describe 'Category API', type: :request do
  ...
  let!(:categories) {create_list(:category, 1, vertical_id: vertical.id, state: 'active')}
  ...
  describe 'POST /categories' do
    # valid payload
    let(:valid_attributes) {{name: 'Learn Elm', vertical_id: vertical.id, state: 'active'}}
    ...
    context 'when the request is invalid' do
      before {post '/categories', params: {vertical_id: vertical.id, state: 'active'}}
