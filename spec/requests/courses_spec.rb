...
RSpec.describe 'Course API', type: :request do
  ...
  let!(:courses) {create_list(:course, 1, category_id: category.id, state: 'active', author: 'Anowa')}
  ...
  describe 'POST /courses' do
    # valid payload
    let(:valid_attributes) {{name: 'Learn Elm', category_id: category.id, state: 'active', author: 'Anowa'}}
    ...
    context 'when the request is invalid' do
      before {post '/courses', params: {category_id: category.id, state: 'active', author: 'Anowa'}}
